...
export default class QueryData extends Component {
  ...
  getSensorAddress = (key) => {
    let sensorInfo = this.state.sensorInfos.filter(item => item.sensorNumber==key);
    this.setState({
  ...
  //选择框提供传感器地址供选择
  getSensorNumber = (key) => {
    let sensorInfo = this.state.sensorInfos.filter(item => item.sensorAddress==key);
    this.setState({
      ...
      <Row gutter={8}>
        ...
        <Col span={5} >
          <Form.Item label="传感器编号"  {...formItemLayout}>
            {getFieldDecorator('sensorNumber', {initialValue:this.state.sensorNumber, rules: [{ required: true, message: '请选择传感器编号'}]})(
              <Select
                placeholder="传感器编号"
                showSearch={true}
                filterOption={(input, option) => option.props.children.toLowerCase().indexOf(input.toLowerCase()) >= 0}
