...
class AddProjectName extends Component {
    ...
    if (allProject.length === 0) {
      ...
      getAllProject(params).then(res => {
        ...
        if (code === 0) {
          ...
          const newArr = data.map(obj => {return obj.projectName});
          this.setState({ allProjectName: newArr });
        } else {
          this.setState({ allProject: [] });
          this.setState({ allProjectName: [] });
  ...
  //查询项目名称是否存在
  projectNameRules = (rule, value, callback) => {
    if(this.state.allProjectName.indexOf(value) > -1){
      callback('项目名称已存在');
    }else{
      if(value === ''){
        callback('请输入项目名称');
        ...
        <Form.Item {...formItemLayout} label="项目名称">
          {getFieldDecorator('name', {
            rules: [
              ...
              { validator: this.projectNameRules }
            ],
