...
class AddSectorName extends Component {
  ...
  //查询子项目名称是否存在
  sectorNameRules = (rule,value,callback) => {
    if(this.state.projectSectorName.indexOf(value) > -1){
      callback('子项目名称已存在');
    }else{
      if(value===''){
        callback('请输入子项目名称');
  ...
  detailForm = (record) => {
    ...
    return (
      <Form layout="horizontal" className={`${styles.stepForm} ${styles.disabled}`}>
        ...
        {/* <Form.Item {...formItemLayout} label="项目监测项">
          {getFieldDecorator('monitorItem', {
            rules: [{ required: true, message: '请输入项目监测项' }],
          })(<Input placeholder="示例: xxx监测项" />)}
        </Form.Item> */}
